...
class MessageView : ConstraintLayout {
    ...
    private var mBackgroundColor: Int = ContextCompat.getColor(context, R.color.teal_200)
    ...
    class Builder{
        ...
        var mListener: MessageViewListener? = null
        ...
        fun listener(listener: MessageViewListener?): Builder {
            mListener = listener
            return this
        }

        fun build(): MessageView{
            return MessageView(mContext.get()!!, this)
        }
    }
    ...
    private fun getMargin(): Int = Screen.dpToPx(20)

    private fun getSecurityArrowMargin(): Int = getMargin() + Screen.dpToPx(2 * WIDTH_ARROW / 3)
    ...
    private fun getArrowHorizontalPositionDependingOnTarget(targetViewLocationOnScreen: RectF?): Int {
        ...
        when {
            ...
            else -> xPosition = Math.round(targetViewLocationOnScreen!!.centerX() - Screen.getAxisXpositionOfViewOnScreen(this))
        }
        return xPosition
    }
    ...
    private fun getArrowVerticalPositionDependingOnTarget(targetViewLocationOnScreen: RectF?): Int {
        ...
        when {
            ...
            else -> yPosition = Math.round(targetViewLocationOnScreen!!.centerY() + Screen.getStatusBarHeight(context) - Screen.getAxisYpositionOfViewOnScreen(this))
        }
        return yPosition
    }

    private fun isOutOfRightBound(targetViewLocationOnScreen: RectF?): Boolean {
        return targetViewLocationOnScreen!!.centerX() > Screen.getAxisXpositionOfViewOnScreen(this) + width - getSecurityArrowMargin()
    }

    private fun isOutOfLeftBound(targetViewLocationOnScreen: RectF?): Boolean {
        return targetViewLocationOnScreen!!.centerX() < Screen.getAxisXpositionOfViewOnScreen(this) + getSecurityArrowMargin()
    }

    private fun isOutOfBottomBound(targetViewLocationOnScreen: RectF?): Boolean {
        return targetViewLocationOnScreen!!.centerY() > Screen.getAxisYpositionOfViewOnScreen(this) + height - getSecurityArrowMargin() - Screen.getStatusBarHeight(context)
    }

    private fun isOutOfTopBound(targetViewLocationOnScreen: RectF?): Boolean {
        return targetViewLocationOnScreen!!.centerY() < Screen.getAxisYpositionOfViewOnScreen(this) + getSecurityArrowMargin() - Screen.getStatusBarHeight(context)
    }
    ...
    private fun drawArrow(canvas: Canvas, arrowPosition: ShowCase.ArrowPosition, targetViewLocationOnScreen: RectF?) {
        ...
        drawRhombus(canvas, paint, xPosition, yPosition, Screen.dpToPx(WIDTH_ARROW))
    }
